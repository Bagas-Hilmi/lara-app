<div style="display: flex; flex-direction: column; gap: 5px;">
    @if ($row->report_status == 0)
        <div class="btn-group" role="group" aria-label="Edit and Delete" style="display: flex;">
            <a class="view btn bg-gradient-secondary btn-sm action-btn" data-id="{{ $row->id_head }}"
                data-bs-toggle="modal" data-bs-target="#replaceDocFormModal">
                <i class="fas fa-edit" style="font-size: 1rem;"></i> Edit
            </a>
            <a class="view btn bg-gradient-danger btn-sm action-btn delete-btn" data-id="{{ $row->id_head }}"
